...
const FinalForm = () => {
  ...
  const sendDataToBackend = async (data: any) => {
    try {
      ...
      alert("✅ Case filed successfully! Case ID: " + response.data.case._id.substring(response.data.case._id.length - 6));
    } catch (error: any) {
      console.error("Error sending data to the backend:", error);
      const errorMessage = error.response?.data?.message || error.message || "Failed to file case. Please try again.";
      alert("❌ Error filing case: " + errorMessage);
    }
  };
